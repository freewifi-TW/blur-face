...
下方「處理紀錄」面板會記錄執行環境（onnxruntime provider、顯示卡型號與驅動）、每個模型實際落在 GPU 或 CPU 與失敗原因、每個檔案完整的階段佔比（解碼 / 偵測 / 追蹤 / 打碼 / 轉bytes / 送編碼 / 收尾，加總約 100%）。處理影片時每隔約 15 秒回報一次即時進度、fps、當前遮蔽框數與記憶體用量；**畫面若卡住不動，會標出「已 N 秒沒有新畫面，可能卡在【某階段】」，一眼看出是解碼、偵測還是編碼卡住。處理變慢或懷疑沒用到 GPU 時，按「複製紀錄」貼出來就能判斷。** 在虛擬桌面 / 串流環境（如 Virtual Desktop）中，OpenCV 的 OpenCL 會被導向虛擬顯示卡而讓影像運算極慢，本工具已一律停用（環境列顯示「OpenCL 關」）。 偵測設定沒變時會沿用同一個偵測器，不會每輪重建 GPU session。
