...
- **特寫／旋轉補救**（預設開啟，`--no-rescue` 停用）：整幀沒抓到臉時，用 YuNet 對轉正（0 / 90 / 270 度）且縮小一半的畫面再試一次，專救「臉比畫面還大、手機橫躺、臉被裁掉一半」的極端特寫。只在沒抓到的幀執行，實測無人照片零誤報
...
| `--no-rescue` | （預設開） | 停用特寫／旋轉補救：預設整幀沒抓到時用 YuNet 對 0/90/270 度、縮小畫布再試 |
...
| 加多尺度 | 8 | 0 | 0 | 不變 |
| 加多尺度 + 特寫補救（v1.2.2 預設） | 8 | 3 | 90 | 補救 0 框 |
