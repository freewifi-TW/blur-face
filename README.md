...
- **旋轉補救**（預設關閉，`--rescue` / GUI 勾選啟用）：整幀沒抓到臉時，把畫面轉 90 / 270 度再用同一套偵測器重跑，救回橫躺、大角度歪斜的臉（躺姿影片、手機橫著拍）。由主偵測器覆核，誤框率與一般偵測相同；命中過的方向會優先嘗試、一中就停，連續橫躺片段每幀只多一次偵測。（v1.2.7 以前為「特寫／旋轉補救」，用低門檻 YuNet 掃縮小畫布，可救特寫大臉但誤框偏多，已改版）
...
| `--rescue` | 關 | 啟用旋轉補救：整幀沒抓到時把畫面轉 90/270 度用主偵測器重跑，救橫躺、大角度歪斜的臉；誤框率與一般偵測相同 |
