...
- **旋轉補救**（預設關閉，`--rescue` / GUI 勾選啟用）：整幀沒抓到臉時，把畫面轉 90 / 270 度再用**臉部模型**重跑（不含頭部模型——它對旋轉畫面上的圓弧皮膚、關節、物體易誤判，且頭部偵測本就耐旋轉），救回橫躺、大角度歪斜的臉（躺姿影片、手機橫著拍），誤框率為 SCRFD 本身的水準；命中過的方向會優先嘗試、一中就停，連續橫躺片段每幀只多一次偵測。（v1.2.7 以前為「特寫／旋轉補救」，用低門檻 YuNet 掃縮小畫布，可救特寫大臉但誤框偏多，已改版）
...
- **管線並行與硬體解碼**：解碼、AI 偵測、打碼＋編碼分屬三個執行緒重疊執行（原本串行時各階段互等，CPU/GPU 都吃不滿）；影片解碼優先使用 FFmpeg 硬體加速（NVDEC / D3D11 / QSV，不支援自動退回軟解）。多偵測器聯集時原始幀只縮圖一次共用
