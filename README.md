...
- **管線並行與硬體解碼**：解碼、AI 偵測、打碼＋編碼分屬三個執行緒重疊執行（原本串行時各階段互等，CPU/GPU 都吃不滿）；多個偵測模型（SCRFD 雙尺度＋頭部）以執行緒池同時送算，偵測時間從各模型相加變成最慢的那個；影片解碼優先使用 FFmpeg 硬體加速（NVDEC / D3D11 / QSV，不支援自動退回軟解）。多偵測器聯集時原始幀只縮圖一次共用
